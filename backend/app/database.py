...
Tables (app-managed auth tables):
...
Note: Live property tables (tbl_PropertyMaster, tbl_PropertyDetails, etc.)
      are mapped in app/property_models.py as read-only models.
"""
...
class RefreshTokenRecord(Base):
    ...
    id = Column(String(255), primary_key=True, index=True)
    user_id = Column(String(255), ForeignKey("users.id", ondelete="NO ACTION"), nullable=False, index=True)
    session_id = Column(String(255), ForeignKey("sessions.id", ondelete="CASCADE"), nullable=False, unique=True)
    token_hash = Column(String(255), unique=True, index=True, nullable=False)  # hashed for storage
    is_revoked = Column(Boolean, default=False)
    created_at = Column(DateTime, default=datetime.utcnow)
    expires_at = Column(DateTime, nullable=False)
    last_used_at = Column(DateTime, nullable=True)
    replaced_by = Column(String(255), nullable=True)   # ID of replacement token (rotation chain)
...
class PasswordResetToken(Base):
    ...
    created_at = Column(DateTime, default=datetime.utcnow)
    expires_at = Column(DateTime, nullable=False)
    ip_address = Column(String(255), nullable=True)  # IP that requested reset
...
class AuditLog(Base):
    ...
    # What
    action = Column(String(255), nullable=False, index=True)   # e.g. "LOGIN", "PASSWORD_RESET"
...
# Unused dataset/ML tables removed
...
def init_db() -> None:
    ...
    try:
        ...
        # 1. Admin Account
        admin_email = "admin@intercity.in"
        ...
        if not admin_exists:
            admin_user = UserRecord(
                ...
                full_name="InterCity Admin Subodh",
            ...
            db.add(admin_user)
            print("[OK] Seeded initial admin user: admin@intercity.in")
        else:
            admin_exists.hashed_password = hashed_admin_pass
            admin_exists.full_name = "InterCity Admin Subodh"
            admin_exists.is_admin = True
            admin_exists.is_verified = True
            admin_exists.role = "admin"
            print("[OK] Updated admin credentials: admin@intercity.in")

        # 2. Standard Demo User Account
        demo_email = "user@intercity.in"
        ...
        if not demo_exists:
            demo_user = UserRecord(
                ...
                full_name="InterCity Demo Agent",
                username="demo_agent",
                is_active=True,
                is_admin=False,
                is_verified=True,
            ...
            db.add(demo_user)
            print("[OK] Seeded demo agent user: user@intercity.in")
        else:
            demo_exists.hashed_password = hashed_demo_pass
            demo_exists.full_name = "InterCity Demo Agent"
            demo_exists.username = "demo_agent"
            demo_exists.is_admin = False
            demo_exists.is_verified = True
            demo_exists.role = "user"
            print("[OK] Updated demo user credentials: user@intercity.in")
...
# NOTE: PropertyRecord and PropertyMetadata have been removed.
# The chatbot now queries live SQL Server tables directly via app/property_models.py.
